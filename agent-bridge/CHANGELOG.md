...
- **Cursor `stop` hook** — allow stop when there is **no pending non-system** mail (even if last tool was `send_message`); block only when actionable inbox remains. Removes empty-inbox stop loops; `afterMCPExecution` still reminds to `listen()`.
- **Cursor Neohive hooks hardening** — `cursor_hook_lib.py` normalizes `MCP: neohive:*` tool names and resolves `.neohive` from `workspace_roots` / env; `cursor-before-prompt.sh` uses live PID checks for online agents; `cursor-stop.sh` allows stop after read-only tools, emits `followup_message` + `agentMessage`; `cursor-post-mcp.sh` reminds listen after mutating tools; `.cursor/hooks.json` uses repo-relative commands + `loop_limit: 8`; `cli.js cursor-hooks` writes relative paths when possible. **Doc:** repo `docs/reference/cursor-neohive-hooks.md`.
